...
import { CustomInstruction, useCustomInstructions } from '@/hooks/use-custom-instructions';
import { useDocumentKnowledge } from '@/hooks/use-document-knowledge';
...
export const LocalLlamaChat = () => {
    ...
    const { activeInstruction } = useCustomInstructions();
    const { searchKnowledge, buildDocumentContext, knowledgeBases } = useDocumentKnowledge();
    ...
    const handleSubmit = async () => {
        ...
        // Use active instruction or selected instruction
        const instructionToUse = selectedInstruction || activeInstruction;

        // Build document context if instruction uses document knowledge
        let documentContext = '';
        if (instructionToUse?.useDocumentContext) {
            documentContext = buildDocumentContext(prompt.trim());
        }
            ...
            <Tabs defaultValue="chat" className="w-full">
                ...
                <TabsContent value="chat" className="space-y-4">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            ...
                            {(selectedInstruction || activeInstruction) && (
                                <Card className="border-primary bg-primary/5">
                                    <CardContent className="p-4">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-medium">
                                                    Active Instruction: {(selectedInstruction || activeInstruction)?.name}
                                                </span>
                                            </div>
                                            <Button
                                                variant="ghost"
                                        ...
                                        <p className="text-sm text-muted-foreground mt-1">
                                            {(selectedInstruction || activeInstruction)?.description}
                                        </p>
                                    </CardContent>
                                </Card>
                            )}
                ...
                <TabsContent value="insights" className="space-y-4">
                    {knowledgeBases.length > 0 ? (
                        <div className="space-y-4">
                            {knowledgeBases.filter(kb => kb.isActive && kb.sources.length > 0).map(kb => (
                                <DocumentInsights key={kb.id} knowledgeBase={kb} />
                            ))}
                            {knowledgeBases.filter(kb => kb.isActive && kb.sources.length > 0).length === 0 && (
                                <Card>
                                    <CardContent className="flex flex-col items-center justify-center py-12">
                                        <BarChart3 className="w-12 h-12 text-muted-foreground/50 mb-4" />
                                        <h3 className="text-lg font-medium mb-2">No Active Knowledge Bases</h3>
                                        <p className="text-muted-foreground text-center max-w-md">
                                            Upload documents in the Documents tab and enable knowledge bases to see AI-powered insights and analysis.
                                        </p>
                                    </CardContent>
                                </Card>
                            )}
                        ...
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-12">
                                ...
                                <p className="text-muted-foreground text-center max-w-md">
                                    Upload your first documents in the Documents tab to start getting AI-powered insights and analysis.
                                </p>
                            </CardContent>
                        </Card>
                    )}
